...
def create_booking(
    ...
    valid_seats = db.query(
        ...
        "WHERE auditorium_id = %s AND seat_id = ANY(%s)",
    ...
    if invalid:
        raise ValidationError(
            f"Seats not in this auditorium: {', '.join(invalid)}"
        )

    # ---- Step 2: one transaction for header + seats + total ----
    # One connection per request, cheap on Lakebase.  A pool is the production
    # optimisation (see SCALE_TO_MILLIONS.md).
    try:
        with db.transaction() as conn:
            with conn.cursor() as cur:
                ...
                if cur.rowcount != len(seat_ids):
                    raise ValidationError(
                        f"Expected {len(seat_ids)} seat rows, "
                        f"got {cur.rowcount}"
                    )
                ...
                booking["seats"] = [
                    dict(zip(seat_cols, row)) for row in cur.fetchall()
                ]
